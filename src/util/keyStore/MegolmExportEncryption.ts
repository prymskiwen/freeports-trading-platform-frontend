 ...
 */
function encodeBase64(uint8Array: Uint8Array | number[]) {
  // Misinterpt the Uint8Array as Latin-1.
  // window.btoa expects a unicode string with codepoints in the range 0-255.
  const latin1String = String.fromCharCode.apply(null, uint8Array as any);
...
async function deriveKeys(
  ...
  const now = new Date();
  console.log(
    `E2e import/export: deriveKeys took ${
      (now as unknown as number) - (start as unknown as number)
    }ms`
  );
  ...
  const result = await Promise.all([aesProm, hmacProm]);
  return result;
}
 ...
 */
function unpackMegolmKeyFile(data: Iterable<number>) {
 ...
 *
 */
async function decryptMegolmKeyFile(
  data: any,
  password: string | undefined
): Promise<string> {
  const body = unpackMegolmKeyFile(data);
  const brand = "freeports";
 ...
 */
function packMegolmKeyFile(data: Uint8Array) {
 ...
 */
async function encryptMegolmKeyFile(
  data: string | undefined,
  password: string | undefined,
  // eslint-disable-next-line camelcase
  options: { kdf_rounds?: any }
...
export { decryptMegolmKeyFile, encryptMegolmKeyFile, readFileAsArrayBuffer };
